    ...
    while 1: # Loop forever so the generator never terminates
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            for batch_sample in batch_samples:
                ...
                center_image_rgb = BGR_to_RGB(center_image)
                center_image = pre_process(center_image_rgb)
                ...
                left_image = cv2.imread(batch_sample[1].split('/')[-1])
                left_image_rgb = BGR_to_RGB(left_image)
                left_image = pre_process(left_image_rgb)
                angles.append(float(left_angle))
                
                right_image = cv2.imread(batch_sample[2].split('/')[-1])
                right_image_rgb = BGR_to_RGB(right_image)
                right_image = pre_process(right_image_rgb)
...
def pre_process(rgbimage):
    yuvimage = cv2.cvtColor(rgbimage, cv2.COLOR_RGB2YUV)
    
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
    claheeqimage = clahe.apply(yuvimage[:,:,0])
    yuvimageclahe = cv2.merge((claheeqimage,yuvimage[:,:,1],yuvimage[:,:,2]))
    claheeqrgb=cv2.cvtColor(yuvimageclahe,cv2.COLOR_YUV2RGB)
    return claheeqrgb
